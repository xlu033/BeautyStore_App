...
//allow the user to use the pagination features
export class PaginationControls extends Component {
    ...
    render() {
        return <div className="m-2">
            ...
            <div className="form-inline justify-content-center mt-3 mb-3">
                <select className="form-control form-control-sm mr-sm-2"
                        onChange={ this.handlePageSizeChange }
                        value={ this.props.pageSize|| this.pageSizes[0] }>
                    { this.pageSizes.map(s =>
                        <option value={s} key={s}>{s} items per page </option>
                    )}
                </select>
                
                <select className="form-control form-control-sm ml-sm-2 colorful-select dropdown-primary"
